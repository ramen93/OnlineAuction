...
if(!empty($e))  {
	$q=mysql_query("select * from user where email='$e'");		//	checked if logged in email-id exists in 'user' database
	$f=mysql_fetch_array($q);
	if($f) {		//	if logged in email-id exist in 'user' database, page is redirected in profile.php
		header("location:profile.php");
	}
	else  {		//	if logged in email-id exist in 'admin' database, page is redirected in admin.php
  ...
  <div id="body">
   <div class="header">
	<div style="color:black; font-size:22px; font-family:Constantia"><br>
	 <table height="80px"><?php if(!empty($message)) echo $message?><tr><td><span style="color:red;"><?php if(!empty($mess)) echo $mess?></span></td>     </tr></table>
	<div><br><br>
